...
public static class DatabaseServer
{
    ...
    {
        var query = $"EXEC FetchVehicle {vechicleID}";
        ...
        while (reader.Read())
        {
            ...
            if (TableName == "Bus" || TableName == "Truck")
            {
                ...
                var vehicleDimensions = new VehicleDimensionsStruct(height, weight, length);
            ...
            else
            {
                ...
                var trunkDimentionsStruct = new TrunkDimentionsStruct(height, width, depth);
    ...
    {
        var query = $"EXEC FetchUser {userName}";
    ...
    {
        var query = "SELECT id FROM Auctions";
        using var reader = ExecuteQuery(query);
        ...
        foreach (var auctionId in auctionIds) Database.Auctions.Add(FetchAuction(auctionId));
    ...
    public static void FetchBidHistory()
    {
        ...
        while (reader.Read())
        {
            ...
            var bidAmound = reader.GetDecimal(2);
    ...
    {
        var query = $"EXEC FetchAuction {auctionId}";
        ...
        while (reader.Read())
        {
            ...
            FetchUser(userName);
            var privateUser = Database.GetPrivateUserByUserName(userName);
            var corporateUser = Database.GetCorporateUserByUserName(userName);
    ...
    public static void InsertUser(string userName, string password, bool CorporateUser, decimal balance, string zipCode,
        decimal credit, string CRNumber)
    {
        var query =
    ...
    public static uint InsertAuction(Vehicle vehicle, ISeller seller, decimal miniumBid)
    {
        ...
        var reader = ExecuteQuery(query);
        while (reader.Read()) auctionId = reader.GetInt32(0);
        return Convert.ToUInt32(auctionId);
    }
